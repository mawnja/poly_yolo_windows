...
void draw_train_poly_loss(char *windows_name, mat_cv* img_src, int img_size, float avg_loss, float max_img_loss, int current_batch, int max_batches,
...
// Poly data augmentation
image image_poly_data_augmentation(mat_cv* mat, int w, int h,
...
void show_polygon_image_cv(image p, const char *name, float *truth, int box_num);
