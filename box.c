...
//0 ~ 360
int poly_calc_vertice_angle(float x, float y)
{
	int a = 0;
	if (x == 0 && y > 0) {
		a = 90;
...
poly_box float_to_poly_box(float *f, int poly_angles, int poly_angle_step, int w, int h)
{
	...
	for (k = 0; k < POLY_MAX_VERTICES; ++k) {
		...
		if (x >= 0 && y >= 0 && x <= 1 && y <= 1) {
			A  = poly_calc_vertice_angle((x-b.x)*w, (y-b.y)*h);
...
poly_box float_to_poly_box_stride(float *f, int stride, int poly_angles, int poly_angle_step, int w, int h)
{
	...
	for (k = 0; k < POLY_MAX_VERTICES; ++k) {
		...
		x = f[(4 + 1 + k*2 + 0)*stride];
		y = f[(4 + 1 + k*2 + 1)*stride];
...
float poly_box_iou_kind(poly_box a, poly_box b, IOU_LOSS iou_kind) 
{
	...
	bb.w = b.w;
	bb.h = b.h;
	return box_iou_kind(aa, bb, iou_kind);
}
...
dxrep dx_poly_box_iou(poly_box pred, poly_box truth, IOU_LOSS iou_loss)
{
	...
	truth_tmp.w = truth.w;
	truth_tmp.h = truth.h;
	return (dx_box_iou(pred_tmp, truth_tmp, iou_loss));
}
