...
polybox_label *read_poly_boxes(char *filename, int *n)
{
#define MAX_STR_LEN  ((POLY_MAX_VERTICES*2 + 5)*8)
	int i, id, count = 0;
	float x,y,w,h,vx, vy;
	char strline[MAX_STR_LEN];
	polybox_label* boxes = (polybox_label*)xcalloc(1, sizeof(polybox_label));
	FILE *file = fopen(filename, "r");
	if (!file) {
	...
	while (fgets(strline, MAX_STR_LEN, file)) {
		//
		boxes = (polybox_label*)xrealloc(boxes, (count+1)*sizeof(polybox_label));
		
		char *p = strchr(strline, '\r');
		if (p != NULL) *p = '\0';
		p = strchr(strline, '\n');
		if (p != NULL) *p = '\0';
		p = strline;
		...
		for(i = 0; i < POLY_MAX_VERTICES; ++i) {
			boxes[count].pts[i].x = -1;
			boxes[count].pts[i].y = -1;
		}
		i = 0;
		while(p - strline < strlen(strline) && *p>='0' && *p<='9') {
			vx = (float)(atof(p));
			p = strchr(p,' ') + 1;
			vy = (float)(atof(p));
			p = strchr(p, ' ');
			if (vx >= 0 && vy >= 0 && vx <= 1 && vy <= 1) {
				boxes[count].pts[i].x = vx;
				boxes[count].pts[i].y = vy;
				i++;
			}
			if (p == NULL) {
				break;
			}
			if (i >= POLY_MAX_VERTICES) {
				break;
			}
			p++;
		}
...
void randomize_polyboxes(polybox_label *b, int n)
{
	int i;
	for (i = 0; i < n; ++i) {
		polybox_label swap = b[i];
...
void correct_polyboxes(polybox_label *boxes, int n, float dx, float dy, float sx, float sy, int flip)
{
    int i,k;
    for(i = 0; i < n; ++i){
		...
		for(k = 0; k < POLY_MAX_VERTICES; ++k) {
			if(boxes[i].pts[k].x == -1 || boxes[i].pts[k].y == -1)
				break;
			boxes[i].pts[k].x = boxes[i].pts[k].x * sx - dx;
			boxes[i].pts[k].y = boxes[i].pts[k].y * sy - dy;
		}
        ...
        if(flip){
			...
			for(k = 0; k < POLY_MAX_VERTICES; ++k) {
				if (boxes[i].pts[k].x == -1 || boxes[i].pts[k].y == -1)
					break;
				boxes[i].pts[k].x = 1. - boxes[i].pts[k].x;
			}
        }
		...
		for(k=0; k<POLY_MAX_VERTICES; ++k) {
			if (boxes[i].pts[k].x == -1 || boxes[i].pts[k].y == -1)
				break;
			boxes[i].pts[k].x = constrain(0, 1, boxes[i].pts[k].x);
			boxes[i].pts[k].y = constrain(0, 1, boxes[i].pts[k].y);
		}
    }
}
...
int fill_polytruth_detection(const char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, int net_w, int net_h, int poly_angles, int poly_angle_step)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
    ...
    int count = 0;
    int i,k;
    polybox_label *boxes = read_poly_boxes(labelpath, &count);
    int min_w_h = 0;
    float lowest_w = 1.F/net_w;
    float lowest_h = 1.F/net_h;
	randomize_polyboxes(boxes, count);
    correct_polyboxes(boxes, count, dx, dy, sx, sy, flip);
    ...
    for (i = 0; i < count; ++i) {
        ...
        truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+0] = x;
        truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+1] = y;
        truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+2] = w;
        truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+3] = h;
        truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+4] = id;
		
		for (k = 0; k < POLY_MAX_VERTICES; ++k) {
			truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+5+k*2+0] = boxes[i].pts[k].x;
			truth[(i-sub)*(5+POLY_MAX_VERTICES*2)+5+k*2+1] = boxes[i].pts[k].y;
		}
...
int fill_truth_detection(const char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy, int net_w, int net_h)
{
    char labelpath[4096];
    replace_image_to_label(path, labelpath);
...
void blend_poly_truth(float *new_truth, int boxes, float *old_truth)
{
	const int t_size = 4 + 1 + POLY_MAX_VERTICES*2;

	int count_new_truth = 0;
	int t, k;
	for (t = 0; t < boxes; ++t) {
		float x = new_truth[t*(4 + 1 + POLY_MAX_VERTICES*2)];
		if (!x) break;
		count_new_truth++;
	}
	for (t = count_new_truth; t < boxes; ++t) {
		...
		for (k = 0; k < POLY_MAX_VERTICES; ++k) {
			new_truth_ptr[5 + k*2 + 0] = old_truth_ptr[5 + k*2 + 0];
			new_truth_ptr[5 + k*2 + 1] = old_truth_ptr[5 + k*2 + 1];
		}
...
void blend_poly_truth_mosaic(float *new_truth, int boxes, float *old_truth, int w, int h, float cut_x, float cut_y, int i_mixup,
    int left_shift, int right_shift, int top_shift, int bot_shift)
{
    ...
    for (t = count_new_truth; t < boxes; ++t) {
        ...
        if (i_mixup == 0) {
			...
			for (k = 0; k < POLY_MAX_VERTICES; ++k) {
				float vx = new_truth_ptr[5+k*2+0];
				float vy = new_truth_ptr[5+k*2+1];
				if (vx == -1 || vy == -1) {
					break;
				}
				new_truth_ptr[5+k*2+0] = vx - (float)(w - cut_x - right_shift)/w;
				new_truth_ptr[5+k*2+1] = vy - (float)(h - cut_y - bot_shift)/h;
			}
        }
        if (i_mixup == 1) {
			...
			for (k = 0; k < POLY_MAX_VERTICES; ++k) {
				...
				new_truth_ptr[5+k*2+0] = vx + (float)(cut_x - left_shift)/w;
				new_truth_ptr[5+k*2+1] = vy - (float)(h - cut_y - bot_shift)/h;
			}
        }
        if (i_mixup == 2) {
			...
			for (k = 0; k < POLY_MAX_VERTICES; ++k) {
				...
				new_truth_ptr[5+k*2+0] = vx - (float)(w - cut_x - right_shift)/w;
				new_truth_ptr[5+k*2+1] = vy + (float)(cut_y - top_shift)/h;
			}
        }
        if (i_mixup == 3) {
            xb = xb + (float)(cut_x - left_shift)/w;
...

        {
            ...
            if (left < 0) {
				...
				for (k = 0; k < POLY_MAX_VERTICES; ++k) {
					...
					new_truth_ptr[5+k*2+0] = vx - diff/2;
				}
            }
            if (right > w) {
            ...
            if (top < 0) {
				...
				for (k = 0; k < POLY_MAX_VERTICES; ++k) {
					...
					new_truth_ptr[5+k*2+1] = vy - diff/2;
				}
            }
            if (bot > h) {
...
data load_poly_data_detection(int n, char **paths, int m, int w, int h, int c, int boxes, int classes, int use_flip, int use_gaussian_noise, int use_blur, int use_mixup,
    float jitter, float resize, float hue, float saturation, float exposure, int mini_batch, int track, int augment_speed, int letter_box, int show_imgs, int poly_angles, int poly_angle_step)
{
    ...
    d.y = make_matrix(n, (5 + POLY_MAX_VERTICES*2)*boxes);
    int i_mixup = 0;
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            float *truth = (float*)xcalloc((5 + POLY_MAX_VERTICES*2)*boxes, sizeof(float));
            ...
            if (src == NULL) {
                printf("\n Error in load_poly_data_detection() - OpenCV \n");
                fflush(stdout);
                if (check_mistakes) {
                    getchar();
                }
            ...
            */
			...
			min_w_h = fill_polytruth_detection(filename, boxes, truth, classes, flip, dx, dy, 1./sx, 1./sy, w, h, poly_angles, poly_angle_step);

            if ((min_w_h/8)<blur && blur>1) blur = min_w_h/8;   // disable blur if one of the objects is too small

            image ai = image_poly_data_augmentation(src, w, h, pleft, ptop, swidth, sheight, flip, dhue, dsat, dexp,
                gaussian_noise, blur, boxes, truth);

            if (use_mixup == 0) {
                d.X.vals[i] = ai.data;
                memcpy(d.y.vals[i], truth, (5 + POLY_MAX_VERTICES*2)*boxes*sizeof(float));
            }
            else if (use_mixup == 1) {
                if (i_mixup == 0) {
                    d.X.vals[i] = ai.data;
                    ...
                    blend_poly_truth(d.y.vals[i], boxes, truth);
                    free_image(old_img);
                    d.X.vals[i] = ai.data;
                }
            }
                ...
                blend_poly_truth_mosaic(d.y.vals[i], boxes, truth, w, h, cut_x[i], cut_y[i], i_mixup, left_shift, right_shift, top_shift, bot_shift);
			...
			//if (1) {
                ...
                int t;
                for (t = 0; t < boxes; ++t) {
                    poly_box b = float_to_poly_box_stride(d.y.vals[i] + t*(4 + 1 + POLY_MAX_VERTICES*2), 1, poly_angles, poly_angle_step, 0, 0);
                    if (!b.x) break;
                ...
                if (show_imgs == 1) {
					...
					show_poly_image_cv(tmp_ai, buff, d.y.vals[i], boxes);
                    wait_until_press_key_cv();
                }
    ...
    for (i_mixup = 0; i_mixup <= use_mixup; i_mixup++) {
        ...
        for (i = 0; i < n; ++i) {
            ...
            */
			...
			min_w_h = fill_truth_detection(filename, boxes, truth, classes, flip, dx, dy, 1./sx, 1./sy, w, h);
...
void *load_thread(void *ptr)
{
    ...
    } else if (a.type == POLY_DETECTION_DATA) {
		*a.d = load_poly_data_detection(a.n, a.paths, a.m, a.w, a.h, a.c, a.num_boxes, a.classes, a.flip, a.gaussian_noise, a.blur, a.mixup, a.jitter, a.resize,
			a.hue, a.saturation, a.exposure, a.mini_batch, a.track, a.augment_speed, a.letter_box, a.show_imgs, a.poly_angles, a.poly_angle_step);
