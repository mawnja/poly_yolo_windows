...
#define POLY_MAX_VERTICES 1000
#define POLY_MAX_ANGLES   24
...
struct layer {
	...
	//poly-yolo
	float poly_rlo_normalizer;
	float poly_alpha_normalizer;
	float poly_prob_normalizer;
	int poly_angle_step;
	int poly_angles;
	//
...
typedef struct polar {
	float x, y;
	float rlo, alpha, prob;
	float dist;
}polar;

typedef struct poly_box {
	float x, y, w, h;
	polar prs[POLY_MAX_ANGLES];
} poly_box;
...
// data.h
typedef enum {
    CLASSIFICATION_DATA, DETECTION_DATA, POLY_DETECTION_DATA, CAPTCHA_DATA, REGION_DATA, IMAGE_DATA, COMPARE_DATA, WRITING_DATA, SWAG_DATA, TAG_DATA, OLD_CLASSIFICATION_DATA, STUDY_DATA, DET_DATA, SUPER_DATA, LETTERBOX_DATA, REGRESSION_DATA, SEGMENTATION_DATA, INSTANCE_DATA, ISEG_DATA
} data_type;
...
typedef struct polybox_label {
	int id;
	float x, y, w, h;
	float left, right, top, bottom;
	point pts[POLY_MAX_VERTICES];  //¶à±ßÐÎ¶¥µã
} polybox_label;
...
LIB_API float validate_poly_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, float *mean_poly_ap);
LIB_API void train_poly_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path);
LIB_API void test_poly_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh, float hier_thresh, int dont_show, int ext_output, int save_labels, char *outfile, int letter_box, int benchmark_layers);
