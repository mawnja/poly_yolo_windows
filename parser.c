...
LAYER_TYPE string_to_layer_type(char * type)
{
	...
	if (strcmp(type, "[poly_yolo]") == 0) return POLY_YOLO;
...
layer parse_poly(list *options, size_params params)
{
    int classes = option_find_int(options, "classes", 20);
    int total = option_find_int(options, "num", 1);
    int num = total;
    ...
    int *mask = parse_yolo_mask(a, &num);
    int max_boxes = option_find_int_quiet(options, "max", 90);
	int angle_step = option_find_int_quiet(options, "poly_angle_step", 15);
    layer l = make_poly_layer(params.batch, params.w, params.h, num, total, mask, classes, max_boxes, angle_step);
    if (l.outputs != params.inputs) {
        printf("Error: l.outputs == params.inputs \n");
        printf("filters= in the [convolutional]-layer doesn't correspond to classes= or mask= in [yolo]-layer \n");
        exit(EXIT_FAILURE);
    ...
    l.iou_normalizer = option_find_float_quiet(options, "iou_normalizer", 0.75);
    l.cls_normalizer = option_find_float_quiet(options, "cls_normalizer", 1);
	l.poly_rlo_normalizer = option_find_float_quiet(options, "poly_rlo_normalizer", 1);
	l.poly_alpha_normalizer = option_find_float_quiet(options, "poly_alpha_normalizer", 1);
	l.poly_prob_normalizer = option_find_float_quiet(options, "poly_prob_normalizer", 1);
...
network parse_network_cfg_custom(char *filename, int batch, int time_steps)
{
    ...
    while(n){
        ...
        if(lt == CONVOLUTIONAL){
            ...
            l.keep_delta_gpu = 1;
        } else if (lt == POLY_YOLO) {
			l = parse_poly(options, params);
			l.keep_delta_gpu = 1;
        ...
        if (n){
            if (l.antialiasing) {
				...
				//printf("AAAA, params.inputs = %d\n", params.inputs);
            }   else {
                params.h = l.out_h;
                params.w = l.out_w;
                params.c = l.out_c;
                params.inputs = l.outputs;
				//printf("BBBB, params.inputs = %d\n", params.inputs);
            }
        }
    ...
    if ((net.w % 32 != 0 || net.h % 32 != 0) && (lt == YOLO || lt == REGION || lt == DETECTION || lt == POLY_YOLO)) {
        printf("\n Warning: width=%d and height=%d in cfg-file must be divisible by 32 for default networks Yolo v1/v2/v3 or poly-yolo!!! \n\n",
            net.w, net.h);
    }
    return net;
}
