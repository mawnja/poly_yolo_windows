...
int resize_network(network *net, int w, int h)
{
    ...
    for (i = 0; i < net->n; ++i){
        ...
        }else if (l.type == POLY_YOLO) {
			resize_poly_layer(&l, w, h);
...
int num_detections(network *net, float thresh)
{
    ...
    for (i = 0; i < net->n; ++i) {
		...
		if (l.type == POLY_YOLO) {
			s += poly_num_detections(l, thresh);
		}
...
int num_detections_batch(network *net, float thresh, int batch)
{
    ...
    for (i = 0; i < net->n; ++i) {
		...
		if (l.type == POLY_YOLO) {
			s += poly_num_detections_batch(l, thresh, batch);
		}
        if (l.type == DETECTION || l.type == REGION) {
            s += l.w*l.h*l.n;
        }
...
detection *make_network_boxes(network *net, float thresh, int *num)
{
    ...
    for (i = 0; i < nboxes; ++i) {
		...
		if (l.type == POLY_YOLO) dets[i].vers = (poly_vertice*)xcalloc(l.poly_angles, sizeof(poly_vertice));
		else dets[i].vers = NULL;
...
void fill_network_boxes(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter)
{
    ...
    for (j = 0; j < net->n; ++j) {
		...
		if (l.type == POLY_YOLO) {
            int count = get_poly_detections(l, w, h, net->w, net->h, thresh, map, relative, dets, letter);
            dets += count;
        }
        if (l.type == GAUSSIAN_YOLO) {
...
void fill_network_boxes_batch(network *net, int w, int h, float thresh, float hier, int *map, int relative, detection *dets, int letter, int batch)
{
    ...
    for (j = 0; j < net->n; ++j) {
		...
		if (l.type == POLY_YOLO) {
            int count = get_poly_detections_batch(l, w, h, net->w, net->h, thresh, map, relative, dets, letter, batch);
            dets += count;
        }
        if (l.type == REGION) {
            custom_get_region_detections(l, w, h, net->w, net->h, thresh, map, hier, relative, dets, letter);
...
void free_detections(detection *dets, int n)
{
    ...
    for (i = 0; i < n; ++i) {
		...
		if (dets[i].vers) free(dets[i].vers);
    }
    free(dets);
}
