...
void train_poly_detector(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear, int dont_show, int calc_map, int mjpeg_port, int show_imgs, int benchmark_layers, char* chart_path)
{
    list *options = read_data_cfg(datacfg);
    char *train_images = option_find_str(options, "train", "data/train.txt");
    char *valid_images = option_find_str(options, "valid", train_images);
    char *backup_directory = option_find_str(options, "backup", "/backup/");
	float mean_poly_ap = 0;
    network net_map;
    if (calc_map) {
    ...
    args.d = &buffer;
    args.type = POLY_DETECTION_DATA;
	args.poly_angles = l.poly_angles;
	args.poly_angle_step = l.poly_angle_step;
    ...
    while (get_current_iteration(net) < net.max_batches) {
        ...
        */
        ...
        if (calc_map && (iteration >= next_map_calc || iteration == net.max_batches)) {
            ...
            iter_map = iteration;
            mean_average_precision = validate_poly_detector_map(datacfg, cfgfile, weightfile, 0.25, 0.5, 0, net.letter_box, &net_map, &mean_poly_ap);// &net_combined);
...
#ifdef OPENCV
		draw_train_poly_loss(windows_name, img, img_size, avg_loss, max_img_loss, iteration, net.max_batches, mean_average_precision, draw_precision, "mAP%", dont_show, mjpeg_port, avg_time, mean_poly_ap);
#endif    // OPENCV
...
void validate_poly_detector(char *datacfg, char *cfgfile, char *weightfile, char *outfile)
{
    int j;
    list *options = read_data_cfg(datacfg);
...
void validate_poly_detector_recall(char *datacfg, char *cfgfile, char *weightfile)
{
    network net = parse_network_cfg_custom(cfgfile, 1, 1);    // set batch=1
    if (weightfile) {
        load_weights(&net, weightfile);
    ...
    for (i = 0; i < m; ++i) {
        ...
        int num_labels = 0;
        polybox_label *truth = read_poly_boxes(labelpath, &num_labels);
        for (k = 0; k < nboxes; ++k) {
...
int poly_detections_comparator(const void *pa, const void *pb)
{
...
int poly_angle(float x, float y)
{
	int a = 0;
	if (x == 0 && y > 0) {
		a = 90;
		...
		a = (int)(atan2(y, x)*180/3.1415926);  //-180 ~ +180
		if (a < 0) {
			a += 360;
		}
	}
...
poly_box polybox_label2polybox(polybox_label truth, int poly_angles, int poly_angle_step, int w, int h) 
{
	int k, A, iA;
	float x, y, dist, diagonal;
	poly_box b;
	...
	for (k = 0; k < POLY_MAX_VERTICES; ++k) {
		...
		if (x >= 0 && y >= 0 && x <= 1 && y <= 1) {
			A = poly_angle((x-b.x)*w, (y-b.y)*h);
			iA= A / poly_angle_step;
			if (iA >= poly_angles) {
				iA = poly_angles - 1;
			}
			dist = sqrtf((x-b.x)*(x-b.x)*w*w + (y-b.y)*(y-b.y)*h*h);
			diagonal = sqrtf(b.w*b.w*w*w + b.h*b.h*h*h);
			if (dist < 1) {
				dist = 1;
			}
			if (dist > b.prs[iA].dist) {
				b.prs[iA].x = x;
				b.prs[iA].y = y;
				b.prs[iA].rlo = dist/diagonal;
				b.prs[iA].alpha = (float)(A - iA*poly_angle_step)/poly_angle_step;
				b.prs[iA].prob = 1.f;
				b.prs[iA].dist = dist;
			}
		}
...
float validate_poly_detector_map(char *datacfg, char *cfgfile, char *weightfile, float thresh_calc_avg_iou, const float iou_thresh, const int map_points, int letter_box, network *existing_net, float *mean_poly_ap)
{
    int j;
    list *options = read_data_cfg(datacfg);
    char *valid_images = option_find_str(options, "valid", "data/train.txt");
    ...
    for (i = nthreads; i < m + nthreads; i += nthreads) {
        ...
        for (t = 0; t < nthreads && i + t - nthreads < m; ++t) {
            ...
            // difficult
            polybox_label *truth_dif = NULL;
            int num_labels_dif = 0;
            if (paths_dif) {
                char *path_dif = paths_dif[image_index];
                char labelpath_dif[4096];
                replace_image_to_label(path_dif, labelpath_dif);
                truth_dif = read_poly_boxes(labelpath_dif, &num_labels_dif);
            }
                    ...
                    if (prob > 0) {
                        ...
                        if (truth_index > -1) {
							...
							if (1) {
								...
								float dsumx = 0.f;
								float dsumy = 0.f;
								poly_box truth_box = polybox_label2polybox(truth[label_index], l.poly_angles, l.poly_angle_step, net.w, net.h);
								for (j = 0; j < l.poly_angles; ++j) {
									dsumx += fabs(dets[i].vers[j].x - truth_box.prs[j].x);
									dsumy += fabs(dets[i].vers[j].y - truth_box.prs[j].y);
									ssumx += fabs(truth_box.prs[j].x);
									ssumy += fabs(truth_box.prs[j].y);
								}
								avg_poly_ap += (dsumx/ssumx + dsumy/ssumy)/2;
								avg_poly_ap_cnt++;
							}
                        }
                        else {
    ...
    // SORT(detections)
    qsort(detections, detections_count, sizeof(box_prob), poly_detections_comparator);
	...
	if (mean_poly_ap != NULL) {
		*mean_poly_ap = avg_poly_ap / avg_poly_ap_cnt;
	}
...
void test_poly_detector(char *datacfg, char *cfgfile, char *weightfile, char *filename, float thresh,
...
void run_poly_detector(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[2], "test")) test_poly_detector(datacfg, cfg, weights, filename, thresh, hier_thresh, dont_show, ext_output, save_labels, outfile, letter_box, benchmark_layers);
    else if (0 == strcmp(argv[2], "train")) train_poly_detector(datacfg, cfg, weights, gpus, ngpus, clear, dont_show, calc_map, mjpeg_port, show_imgs, benchmark_layers, chart_path);
    else if (0 == strcmp(argv[2], "valid")) validate_poly_detector(datacfg, cfg, weights, outfile);
    else if (0 == strcmp(argv[2], "recall")) validate_poly_detector_recall(datacfg, cfg, weights);
    else if (0 == strcmp(argv[2], "map")) validate_poly_detector_map(datacfg, cfg, weights, thresh, iou_thresh, map_points, letter_box, NULL, NULL);
