...
// ----------------------------------------
extern "C" void show_poly_image_cv(image p, const char *name, float *truth, int box_num)
{
	try {
		int t,k;
		image copy = copy_image(p);
		...
		for (t = 0; t < box_num; ++t) {
			...
			float x = truth[t*(5 + POLY_MAX_VERTICES*2) + 0];
			float y = truth[t*(5 + POLY_MAX_VERTICES*2) + 1];

			int k1 = 0;
			cv::Point pt0, pt1, pt2;
			
			for (k = 0; k < POLY_MAX_VERTICES; ++k) {
				float x1 = truth[t*(5 + POLY_MAX_VERTICES*2) + 5 + 2*k + 0];
				float y1 = truth[t*(5 + POLY_MAX_VERTICES*2) + 5 + 2*k + 1];
				float ds = sqrtf((x1-x)*(x1-x)*mat.cols*mat.cols + (y1-y)*(y1-y)*mat.rows*mat.rows);
				k1++;
				if (x1 >= 0 && y1 >= 0 && x1 <= 1 && y1 <= 1 && ds > 1.0f) {
					pt1.x = (int)(x1 * mat.cols);
					pt1.y = (int)(y1 * mat.rows);
					pt0 = pt1;
					//printf("x1=%0.4f, y1=%0.4f, dist1=%0.4f\n", x1, y1, ds);
					break;
				}
			}

			for (k = k1; k < POLY_MAX_VERTICES; ++k) {
				float x2 = truth[t*(5+POLY_MAX_VERTICES*2) + 5 + 2*k + 0];
				float y2 = truth[t*(5+POLY_MAX_VERTICES*2) + 5 + 2*k + 1];
				float ds = sqrt((x2-x)*(x2-x)*mat.cols*mat.cols + (y2-y)*(y2-y)*mat.rows*mat.rows);
				if (x2 >= 0 && y2 >= 0 && x2 <= 1 && y2 <= 1 && ds > 1.0f) {
					pt2.x = (int)(x2 * mat.cols);
					pt2.y = (int)(y2 * mat.rows);
					cv::line(mat, pt2, pt1, cv::Scalar(0, 0, 255), 1, 8, 0);
					pt1 = pt2;
					//printf("x2=%0.4f, y2=%0.4f, dist2=%0.4f\n", x2, y2, ds);
				}
			}
			if (k > k1) {
				cv::line(mat, pt0, pt1, cv::Scalar(0, 0, 255), 1, 8, 0);
			}
		}
		//cv::imwrite(name,mat);
		printf("box_num = %d\n", t);
    ...
    float precision, int draw_precision, char *accuracy_name, int dont_show, int mjpeg_port, double time_remaining)
{
    try {
        ...
        // precision
        if (draw_precision) {
			static float old_poly_precision = 0;
...
extern "C" void draw_train_poly_loss(char *windows_name, mat_cv* img_src, int img_size, float avg_loss, float max_img_loss, int current_batch, int max_batches,
    float box_precision, int draw_precision, char *accuracy_name, int dont_show, int mjpeg_port, double time_remaining, float poly_precision)
{
    try {
        cv::Mat &img = *(cv::Mat*)img_src;
        int img_offset = 60;
        ...
        if (draw_precision) {
			...
			static float old_poly_precision = 0;
			static float max_poly_precision = 0;
            static float old_box_precision = 0;
            static float max_box_precision = 0;
            static int iteration_old = 0;
            static int text_iteration_old = 0;
			...
			if (iteration_old != 0) {
            	cv::line(img,
					cv::Point(img_offset + draw_size * (float)iteration_old / max_batches, draw_size * (1 - old_box_precision)),
					cv::Point(img_offset + draw_size * (float)current_batch / max_batches, draw_size * (1 - box_precision)),
					CV_RGB(255, 0, 0), 1, 8, 0);
				cv::line(img,
					cv::Point(img_offset + draw_size * (float)iteration_old / max_batches, draw_size * (1 - old_poly_precision)),
					cv::Point(img_offset + draw_size * (float)current_batch / max_batches, draw_size * (1 - poly_precision)),
					CV_RGB(255, 0, 0), 1, 8, 0);
			}

            sprintf(char_buff, "%2.1f%% ", box_precision * 100);
            cv::putText(img, char_buff, cv::Point(10, 28), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255), 5, CV_AA);
            cv::putText(img, char_buff, cv::Point(10, 28), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(200, 0, 0), 1, CV_AA);

            if ((std::fabs(old_box_precision - box_precision) > 0.1)  || (max_box_precision < box_precision) || (current_batch - text_iteration_old) >= max_batches / 10) {
                text_iteration_old = current_batch;
                max_box_precision = std::max(max_box_precision, box_precision);
                sprintf(char_buff, "m%2.0f%% ", box_precision * 100);
                cv::putText(img, char_buff, cv::Point(pt1.x - 30, draw_size * (1 - box_precision) + 15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255), 5, CV_AA);
                cv::putText(img, char_buff, cv::Point(pt1.x - 30, draw_size * (1 - box_precision) + 15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(200, 0, 0), 1, CV_AA);
            }
			if ((std::fabs(old_poly_precision - poly_precision) > 0.1) || (max_poly_precision < poly_precision) || (current_batch - text_iteration_old) >= max_batches / 10) {
				text_iteration_old = current_batch;
				max_poly_precision = std::max(max_poly_precision, poly_precision);
				sprintf(char_buff, "e%2.0f%% ", poly_precision * 100);
				cv::putText(img, char_buff, cv::Point(pt1.x - 30, draw_size * (1 - poly_precision) + 15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(255, 255, 255), 5, CV_AA);
				cv::putText(img, char_buff, cv::Point(pt1.x - 30, draw_size * (1 - poly_precision) + 15), cv::FONT_HERSHEY_COMPLEX_SMALL, 0.7, CV_RGB(200, 0, 0), 1, CV_AA);
			}

			old_poly_precision = poly_precision;
            old_box_precision = box_precision;
            iteration_old = current_batch;
        }
        sprintf(char_buff, "current avg loss = %2.4f    iteration = %d    approx. time left = %2.2f hours", avg_loss, current_batch, time_remaining);
        pt1.x = 15, pt1.y = draw_size + 18;
...
// ----------------------------------------
extern "C" image image_poly_data_augmentation(mat_cv* mat, int w, int h,
        ...
        if (blur) {
            ...
            if (blur == 1) {
                ...
                for (t = 0; t < num_boxes; ++t) {
                    poly_box b = float_to_poly_box_stride(truth + t*(4 + 1 + POLY_MAX_VERTICES*2), 1, 0, 0, 0, 0);
                    if (!b.x) break;
